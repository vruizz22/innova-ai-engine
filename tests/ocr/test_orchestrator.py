...
def test_orchestrator_uses_gemini_when_high_confidence() -> None:
    ...
    high = OcrResult(latex_steps=["x=3"], overall_confidence=0.9, provider=OcrProvider.GEMINI)
...
def test_orchestrator_escalates_to_claude_on_low_confidence() -> None:
    ...
    low = OcrResult(latex_steps=[], overall_confidence=0.5, provider=OcrProvider.GEMINI)
    high = OcrResult(
        latex_steps=["x+2=5", "x=3"], overall_confidence=0.88, provider=OcrProvider.CLAUDE
    )
...
def test_orchestrator_keeps_gemini_if_claude_worse() -> None:
    ...
    gemini_r = OcrResult(latex_steps=["x=1"], overall_confidence=0.4, provider=OcrProvider.GEMINI)
    claude_r = OcrResult(latex_steps=[], overall_confidence=0.3, provider=OcrProvider.CLAUDE)
