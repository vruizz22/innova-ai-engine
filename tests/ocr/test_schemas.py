...
def test_ocr_result_valid() -> None:
    r = OcrResult(latex_steps=["x+1=3", "x=2"], overall_confidence=0.9, provider=OcrProvider.GEMINI)
...
def test_ocr_result_invalid_confidence() -> None:
    with pytest.raises(ValidationError):
        OcrResult(latex_steps=[], overall_confidence=1.5, provider=OcrProvider.GEMINI)
