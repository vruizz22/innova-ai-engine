...
async def run_hourly_alerts(*, repo: AlertRepositoryPort, settings: Settings) -> AlertRunOutcome:
    ...
    candidates += detect_student_drop(mastery, drop_threshold=settings.alert_student_drop_trend)
    candidates += detect_unit_off_track(mastery, pknown_floor=settings.alert_unit_off_track_floor)
    ...
    for candidate in candidates:
        if await repo.insert_alert_if_absent(candidate):
            inserted += 1
            by_type[candidate.alert_type] = by_type.get(candidate.alert_type, 0) + 1
    ...
    return AlertRunOutcome(candidates=len(candidates), inserted=inserted, by_type=by_type)
